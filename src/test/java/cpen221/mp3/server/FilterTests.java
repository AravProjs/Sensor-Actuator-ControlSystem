...
public class FilterTests{
    ...
    @Test
    public void testInvalidFieldConstructor() {
        assertThrows(IllegalArgumentException.class, () -> new Filter("invalid", DoubleOperator.GREATER_THAN, 10.0));
    }

    @Test
    public void testSiftWithNoMatchingEvents() {
        Event event = new ActuatorEvent(0.0, 0, 0, "", true);
        Filter filter = new Filter(BooleanOperator.EQUALS, false);
        assertNull(filter.sift(event));
    }

    @Test
    public void testSiftWithMatchingEvents() {
        Event event1 = new SensorEvent(0.0, 0, 0, "", 25.0);
        Event event2 = new SensorEvent(0.0, 0, 0, "", 30.0);
        List<Event> eventsList = new ArrayList<>();
        eventsList.add(event1);
        eventsList.add(event2);

        Filter filter = new Filter("value", DoubleOperator.GREATER_THAN_OR_EQUALS, 25.0);
        List<Event> siftedEvents = filter.sift(eventsList);

        assertEquals(2, siftedEvents.size());
        assertTrue(siftedEvents.contains(event1));
        assertTrue(siftedEvents.contains(event2));
    }

    @Test
    public void testComplexFilterSatisfies() {
        Event sensorEvent = new SensorEvent(0.0, 0, 0, "", 20.0);
        Filter sensorValueFilter = new Filter("value", GREATER_THAN_OR_EQUALS, 15.0);
        Filter sensorTSFilter = new Filter("timestamp", LESS_THAN, 1.0);
        List<Filter> filterList = new ArrayList<>();
        filterList.add(sensorValueFilter);
        filterList.add(sensorTSFilter);
        Filter complexFilter = new Filter(filterList);
